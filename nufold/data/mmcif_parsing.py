...
import nufold.common.base_constants as base_constants
    ...
    try:
        ...
        for chain_id, seq_info in valid_chains.items():
            ...
            for monomer in seq_info:
                code = base_constants.restype_3to1.get(monomer.id, "X")
...
def _get_rna_chains(
    *, parsed_info: Mapping[str, Any]
    ...
    for entity_id, seq_info in polymers.items():
        ...
        if any(
            [
                "RNA linking" in chem_comps[monomer.id]["_chem_comp.type"]
                for monomer in seq_info
            ]
        ):
...
def get_atom_coords(
    ...
    all_atom_positions = np.zeros(
        [num_res, base_constants.atom_type_num, 3], dtype=np.float32
    )
    all_atom_mask = np.zeros(
        [num_res, base_constants.atom_type_num], dtype=np.float32
    )
    for res_index in range(num_res):
        pos = np.zeros([base_constants.atom_type_num, 3], dtype=np.float32)
        mask = np.zeros([base_constants.atom_type_num], dtype=np.float32)
        ...
        if not res_at_position.is_missing:
            ...
            for atom in res.get_atoms():
                ...
                if atom_name in base_constants.atom_order.keys():
                    pos[base_constants.atom_order[atom_name]] = [x, y, z]
                    mask[base_constants.atom_order[atom_name]] = 1.0
