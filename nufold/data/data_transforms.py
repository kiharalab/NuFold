...
from nufold.config import NUM_RES, NUM_EXTRA_SEQ, NUM_TEMPLATES, NUM_MSA_SEQ
from nufold.common import base_constants as rc
...
def fix_templates_aatype(protein):
    ...
    if(num_templates > 0):
        ...
        new_order_list = rc.rev_restype_order_with_x_and_gap
...
def correct_msa_restypes(protein):
    ...
    new_order_list = rc.map_id_to_out_aatype
    new_order = torch.tensor(
        [new_order_list] * protein["msa"].shape[1], device=protein["msa"].device,
    ).transpose(0, 1)
    protein["msa"] = torch.gather(new_order, 0, protein["msa"])

    perm_matrix = np.zeros((6, 6), dtype=np.float32)  # was 22
...
@curry1
def nearest_neighbor_clusters(protein, gap_agreement_weight=0.0):  # TODO
    weights = torch.cat(
        [
...
def summarize_clusters(protein):
    ...
    msa_sum = csum(mask[:, :, None] * make_one_hot(protein["extra_msa"], 7))  # was 23
    msa_sum += make_one_hot(protein["msa"], 7)  # Original sequence  # was 23
...
@curry1
def make_atom_p(protein, prefix=""):
    """Create p position and mask."""
    (
        protein[prefix + "atom_p"],
        protein[prefix + "atom_p_mask"],
    ) = atom_pick_fn(
        protein["template_aatype" if prefix else "aatype"],
        protein[prefix + "all_atom_positions"],
        protein["template_all_atom_mask" if prefix else "all_atom_mask"],
...
@curry1
def make_atom_c4p(protein, prefix=""):
    """Create p position and mask."""
    (
        protein[prefix + "atom_c4p"],
        protein[prefix + "atom_c4p_mask"],
    ) = atom_pick_fn(
        protein["template_aatype" if prefix else "aatype"],
        protein[prefix + "all_atom_positions"],
        protein["template_all_atom_mask" if prefix else "all_atom_mask"],
...
def make_hhblits_profile(protein):
    ...
    msa_one_hot = make_one_hot(protein["msa"], 6)  # was 22
...
def make_masked_msa(protein, config, replace_fraction):
    ...
    categorical_probs = (
        ...
        + config.same_prob * make_one_hot(protein["msa"], 6)
    )
...
def make_msa_feat(protein):
    ...
    aatype_1hot = make_one_hot(protein["aatype"], 5)
    ...
    msa_1hot = make_one_hot(protein["msa"], 7)
...
def make_atom23_masks(protein):
    ...
    residx_atom23_to_atom28 = restype_atom23_to_atom28[protein_aatype]
    residx_atom23_mask = restype_atom23_mask[protein_aatype]

    protein["atom23_atom_exists"] = residx_atom23_mask
    protein["residx_atom23_to_atom28"] = residx_atom23_to_atom28.long()

    # create the gather indices for mapping back
    residx_atom28_to_atom23 = restype_atom28_to_atom23[protein_aatype]
    protein["residx_atom28_to_atom23"] = residx_atom28_to_atom23.long()

    # create the corresponding mask
    restype_atom28_mask = torch.zeros(
        [5, 28], dtype=torch.float32, device=protein["aatype"].device
    )
    ...
    residx_atom28_mask = restype_atom28_mask[protein_aatype]
    protein["atom28_atom_exists"] = residx_atom28_mask
...
def make_atom23_positions(protein):
    """Constructs denser atom positions (23 dimensions instead of 28)."""
    residx_atom23_mask = protein["atom23_atom_exists"]
    residx_atom23_to_atom28 = protein["residx_atom23_to_atom28"]
    ...
    protein["atom23_atom_exists"] = residx_atom23_mask
    protein["atom23_gt_exists"] = residx_atom23_gt_mask
    protein["atom23_gt_positions"] = residx_atom23_gt_positions
    ...
    for resname, swap in rc.residue_atom_renaming_swaps.items():
        correspondences = torch.arange(
            23, device=protein["all_atom_mask"].device
        )
        for source_atom_swap, target_atom_swap in swap.items():
            source_index = rc.restype_name_to_atom23_names[resname].index(
                source_atom_swap
            )
            target_index = rc.restype_name_to_atom23_names[resname].index(
                target_atom_swap
            )
            correspondences[source_index] = target_index
            correspondences[target_index] = source_index
            renaming_matrix = protein["all_atom_mask"].new_zeros((23, 23))
    ...
    # Apply it to the ground truth positions. shape (num_res, 28, 3).
    alternative_gt_positions = torch.einsum(
        "...rac,...rab->...rbc", residx_atom23_gt_positions, renaming_transform
    )
    protein["atom23_alt_gt_positions"] = alternative_gt_positions

    # Create the mask for the alternative ground truth (differs from the
    # ground truth mask, if only one of the atoms in an ambiguous pair has a
    # ground truth position).
    alternative_gt_mask = torch.einsum(
        "...ra,...rab->...rb", residx_atom23_gt_mask, renaming_transform
    )
    protein["atom23_alt_gt_exists"] = alternative_gt_mask

    # Create an ambiguous atoms mask.  shape: (5, 23).
    restype_atom23_is_ambiguous = protein["all_atom_mask"].new_zeros((5, 23))
    for resname, swap in rc.residue_atom_renaming_swaps.items():
        for atom_name1, atom_name2 in swap.items():
            restype = rc.restype_order[rc.restype_3to1[resname]]
            atom_idx1 = rc.restype_name_to_atom23_names[resname].index(
                atom_name1
            )
            atom_idx2 = rc.restype_name_to_atom23_names[resname].index(
                atom_name2
            )
            restype_atom23_is_ambiguous[restype, atom_idx1] = 1
            restype_atom23_is_ambiguous[restype, atom_idx2] = 1

    # From this create an ambiguous_mask for the given sequence.
    protein["atom23_atom_is_ambiguous"] = restype_atom23_is_ambiguous[
        protein["aatype"]
    ]
...
def atom28_to_frames(protein, eps=1e-8):
    aatype: torch.Tensor = protein["aatype"]
    all_atom_positions: torch.Tensor = protein["all_atom_positions"]
    all_atom_mask: torch.Tensor = protein["all_atom_mask"]
...
def atom28_to_torsion_angles(
    ...
    Returns:
        ...
        "(prefix)torsion_angles_mask" ([*, N_res, 7])
            Torsion angles mask
    """
    aatype: torch.Tensor = protein[prefix + "aatype"]
    all_atom_positions: torch.Tensor = protein[prefix + "all_atom_positions"]
    all_atom_mask: torch.Tensor = protein[prefix + "all_atom_mask"]

    aatype = torch.clamp(aatype, max=NUM_AA)

    pad = all_atom_positions.new_zeros(
        [*all_atom_positions.shape[:-3], 1, NUM_ATOM_37, 3]
    )  # [0 1 28 3]
    ...
    pad = all_atom_mask.new_zeros([*all_atom_mask.shape[:-2], 1, NUM_ATOM_37])  # [0 1 28]
...
def atom28_to_2d_angles(
    ...
    c1p_idx = rc.atom_order["C1'"]
    c1p = torch.where(
        is_prn[..., None].expand(*((-1,) * len(is_prn.shape)), 3),
        all_atom_positions[..., c1p_idx, :],
        all_atom_positions[..., c1p_idx, :],
    ).unsqueeze(-2)
    fatoms = torch.cat(
        [
            c1p[..., None, :, :].repeat(1, aalen, 1, 1),    # C1'  i
            n19[..., None, :, :].repeat(1, aalen, 1, 1),    # N1/9 i
            n19[..., None, :, :, :].repeat(aalen, 1, 1, 1), # N1/9 j
            c1p[..., None, :, :, :].repeat(aalen, 1, 1, 1), # C1'  j
        ], dim=-2,
    )
    b = fatoms[..., 1:, :] - fatoms[..., :-1, :]
    b[..., 0, :] *= -1
    b[..., 1, :] /= torch.linalg.norm(b[..., 1, :], dim=-1, keepdim=True) + epsilon

    v = b[..., 0, :] - (b[..., 0, :] * b[..., 1, :]).sum(-1, keepdims=True) * b[..., 1, :]
    w = b[..., 2, :] - (b[..., 2, :] * b[..., 1, :]).sum(-1, keepdims=True) * b[..., 1, :]
    x = (v * w).sum(-1, keepdims=True)
    y = (torch.cross(b[..., 1, :], v) * w).sum(-1, keepdims=True)

    # [L, L, 1]  range: -pi to pi
    res = torch.atan2(y, x)
    protein["angle_2d"] = res
    return protein