...
from nufold.model.openfold.feats import (
    pseudo_beta_fn,
...
from nufold.model.openfold.embedders import (
    InputEmbedder,
    RecyclingEmbedder,
...
from nufold.model.openfold.evoformer import (
    EvoformerStack,
    ExtraMSAStack,
)
from nufold.model.openfold.heads import AuxiliaryHeads
from nufold.common import base_constants
from nufold.model.openfold.structure_module import StructureModule
from nufold.model.openfold.template import (
    TemplatePairStack,
...
class Nufold(nn.Module):
    """Nufold model"""
    def __init__(self, config):
        super(Nufold, self).__init__()
        ...
        self.ss_embedder = TemplatePairEmbedder(
            **ss_config["ss_pair_embedder"],
        )
        self.ss_pair_stack = TemplatePairStack(
            **ss_config["ss_pair_stack"],
        )
        self.ss_pointwise_att = TemplatePointwiseAttention(
            **ss_config["ss_pointwise_attention"],
        )
    ...
    def embed_ss(self, batch, z, pair_mask, n_d=1):
        # Embed the ss
        ss_embeds = []
        n_ss = batch["ss_map"].shape[n_d]
        for i in range(n_ss):
            idx = batch["ss_map"].new_tensor(i, dtype=int)
            single_ss_feats = tensor_tree_map(
                lambda t: torch.index_select(t, n_d, idx),
                {"ss_map": batch["ss_map"]},
            )
            single_ss_feats["aatype"] = batch["aatype"]

            t = build_ss_pair_feat(single_ss_feats).to(z.dtype)
            t = self.ss_embedder(t)
            ss_embeds.append({"pair": t})

        ss_embeds = dict_multimap(
            partial(torch.cat, dim=n_d),
            ss_embeds,
        )

        # [*, S_t, N, N, C_z]
        t = self.ss_pair_stack(
            ss_embeds["pair"],
    ...
    def iteration(self, feats, m_1_prev, z_prev, x_prev, _recycle=True):
        ...
        batch_dims = feats["target_feat"].shape[:-2]  # 1
        no_batch_dims = len(batch_dims)               # 1
        n = feats["target_feat"].shape[-2]            # 256
        n_seq = feats["msa_feat"].shape[-3]           # 128
        ...
        if None in [m_1_prev, z_prev, x_prev]:
            ...
            # [*, N, 3]
            x_prev = z.new_zeros(
                (*batch_dims, n, base_constants.atom_type_num, 3),
                requires_grad=False,
            )
        ...
        # Embed the SS input + merge with pair embeddings
        if self.config.ss.enabled:
            ss_embeds = self.embed_ss(
                feats,
                z,
                pair_mask.to(dtype=z.dtype),
                no_batch_dims,
            )
            z = z + ss_embeds["ss_pair_embedding"]
        ...
        outputs["final_atom_positions"] = atom23_to_atom28(
        ...
        Args:
            batch:
                ...
                Features (without the recycling dimension):
                    ...
                    "template_all_atom_positions" ([*, N_templ, N_res, 28, 3])
                        Template atom coordinates in atom28 format
                    "template_all_atom_mask" ([*, N_templ, N_res, 28])
        ...
        for cycle_no in range(num_iters):
            ...
            with torch.set_grad_enabled(is_grad_enabled and is_final_iter):
                ...
                if not is_final_iter:
                    _o = tensor_tree_map(lambda x: x.detach().clone(), outputs)
                    _rec.append(_o)

        # Run auxiliary heads
        outputs.update(self.aux_heads(outputs))
        # append all recycling results (and keep the dict shape hopefully)
        outputs["recycling"] = _rec
        return outputs